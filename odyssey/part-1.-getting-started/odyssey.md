...
| **DB**   | MySQL 5.7 / 8.0 이상 (UTF-8 환경). 다른 DBMS 드라이버는 `lib/` 폴더를 참고하여 교체                                                                                |
...
2. **`backup=DAILY` + 보관 정책 부재** — 일별 로그 테이블이 무한 누적됩니다. 별도 보관 주기 정책을 함께 마련하세요.
...
{% hint style="info" %}
사용하지 않는 메시지 종류는 `false`로 두면 불필요한 DB 폴링이 줄어 DB 부하가 낮아집니다. `fetch`(로그 이관)는 HA 운영 시 `true` 권장입니다.
{% endhint %}
...
{% hint style="info" %}
계약된 발송 경로와 **반드시 일치**해야 합니다. 경로가 이중화된 경우 장애 시에만 수동 변경하며, 평상시에는 계약 주 경로를 유지합니다.
{% endhint %}
...
1. **`max-cnt`를 계약 한도보다 크게 설정** — 게이트웨이가 초과분을 차단하여 일정 시간 발송이 실패합니다.
...
`HA_AUTO_SWITCH`는 AS 모드에서 SLAVE가 활성된 동안 MASTER가 복구되면 자동으로 역할을 되돌릴지 여부입니다. **`N` (수동 전환) 유지를 권장**합니다.
...
Peer 노드 감시 주기, Failback 안정화 대기, Split-Brain 보호 동작 등을 제어합니다. **기본값 유지를 강력히 권장**합니다.
...
* **`DB_LEASE_TTL_MS` / `RENEW_INTERVAL_MS`**: 갱신 주기는 TTL의 1/3 \~ 1/5 사이를 권장합니다.
* **`DB_LEASE_FAIL_CLOSED=Y`**: DB 연결이 끊기면 본 노드가 즉시 정지해 split-brain 위험을 차단합니다.

### 로드밸런싱 (AA 모드 전용)

`LOAD_BALANCE_PERCENT`는 본 노드가 처리할 메시지 비율(%)이며, **양 노드의 합이 정확히 100**이 되어야 합니다.
...
`ENFORCE_LOAD_BALANCE_RATIO=Y` 설정 시 비율을 어기는 메시지 fetch를 차단합니다. `N`은 권고치로만 사용합니다.
...
2. **`LOAD_BALANCE_PERCENT` 합이 100이 아님** — 양 노드 값의 합을 반드시 100으로 맞추세요.
...
{% hint style="info" %}
Poseidon 연동을 사용하는 경우 **고객사 방화벽에 Poseidon 서버 IP를 등록**해 주셔야 합니다. Odyssey가 주기적으로 외부의 Poseidon 클라우드(`poseidon.url` 항목에 명시된 호스트)로 HTTPS 요청을 송신해야 하므로 outbound 통신이 차단되어 있는 경우 연동이 동작하지 않습니다.
{% endhint %}
...
{% hint style="info" %}
파일이 2개, 3개인 경우 `REPLACE_FILE_NAME1` / `REPLACE_FILE_NAME2` / `REPLACE_FILE_NAME3` 을 각각 사용하며, `FILE_NAME1~3` 컬럼에 대응되는 파일명을 저장합니다.
{% endhint %}
...
* **시간대별 발송 추이**: 00시 \~ 23시 시간대별 발송 추이 차트
* **템플릿별 발송 추이**: RCS·Kakao 알림톡 템플릿 코드별 발송 건수
...
* **JDBC URL / 사용자명**: 보안상 읽기 전용으로 표시됩니다
...
* **KT RCS 세션**: Send URL, Auth URL, 인증 정보
