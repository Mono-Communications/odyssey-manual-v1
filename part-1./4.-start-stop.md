...
배포 패키지에는 사전 빌드된 thin jar(<mark style="color:red;">`odyssey-app-1.0.0.jar`</mark>, <mark style="color:red;">`odyssey-pipeline-1.0.0.jar`</mark>)와 번들 JRE(**`jre/`**), 외부 라이브러리(**`lib/`**)가 포함되어 있어, 별도 빌드 없이 바로 기동할 수 있습니다.
...
<figure><img src="../.gitbook/assets/24_installService.webp" alt="그림 24. installService.bat 실행 — 모드 선택 후 서비스 자동 등록 + 시작"><figcaption><p>그림 24. installService.bat 실행 — 모드 선택 후 서비스 자동 등록 + 시작</p></figcaption></figure>
...
<mark style="color:red;">`log/agent.log`</mark>(또는 모드별 <mark style="color:red;">`log/odyssey-app.out`</mark> / <mark style="color:red;">`log/odyssey-pipeline.out`</mark>)에 다음과 유사한 메시지가 출력됩니다.
