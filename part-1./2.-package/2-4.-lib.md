...
<figure><img src="../../.gitbook/assets/07_lib_내부.png" alt="그림 7. lib 폴더 내부 (외부 라이브러리 약 130개)"><figcaption><p>그림 7. lib 폴더 내부 (외부 라이브러리 약 130개)</p></figcaption></figure>
...
# 2. 호환 드라이버 추가
cp /tmp/mysql-connector-java-5.1.49.jar
...
{% hint style="info" %}
**드라이버 교체 시&#x20;**<mark style="color:red;">**`application.yml`**</mark>**&#x20;파일도 함께 확인**해야 합니다. <mark style="color:green;">`driver-class-name`</mark> 과 <mark style="color:green;">`jdbc-url`</mark> 옵션이 버전마다 다릅니다. ([10. DB 설정](../../part-2./10.-db.md) 참고)
{% endhint %}
...
{% hint style="info" %}
**`lib/`** 안에는 **외부 라이브러리만** 들어있습니다. <mark style="color:red;">`odyssey-*.jar`</mark> 같은 자사 코드는 **패키지 루트**에 위치하며, 임의로 변경하지 마세요.
{% endhint %}
